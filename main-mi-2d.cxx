...
void calc_mutual_information(MutualInformation2D *mi, std::fstream *file_out, std::fstream *file_out_nans, std::string metric = "NONE", long starting_point = -1, long lag = -1){
...
//   std::cerr << "1" << std::endl;  
  ...
  if( CTGlobal::metric == "X" || CTGlobal::metric == "ALL" ){
     file_x = LocalUtils::create_output_file( CTGlobal::basename, "X", "mi.dat");
     file_x_nans = LocalUtils::create_output_file( CTGlobal::basename, "X", "NONNANS.mi.dat");
  }
  if( CTGlobal::metric == "Y" || CTGlobal::metric == "ALL" ){
     file_y = LocalUtils::create_output_file( CTGlobal::basename, "Y", "mi.dat");
     file_y_nans = LocalUtils::create_output_file( CTGlobal::basename, "Y", "NONNANS.mi.dat");
  }
  if( CTGlobal::metric == "ANGLE" || CTGlobal::metric == "ALL" ){
     file_angle = LocalUtils::create_output_file( CTGlobal::basename, "ANGLE", "mi.dat");
     file_angle_nans = LocalUtils::create_output_file( CTGlobal::basename, "ANGLE", "NONNANS.mi.dat");
  }
